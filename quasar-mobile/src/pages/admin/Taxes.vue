...
export default defineComponent({
  ...
  setup () {
    ...
    const loadTaxes = async () => {
      try {
        loading.value = true
        const response = await api.get(`/admin/${authStore.businessId}/taxes`)
    ...
    const saveTax = async () => {
      try {
        saving.value = true
        if (isEditing.value) {
          await api.put(`/admin/${authStore.businessId}/taxes/${selectedTax.value.id}`, taxForm.value)
          $q.notify({
            color: 'positive',
          ...
          await api.post(`/admin/${authStore.businessId}/taxes`, taxForm.value)
          $q.notify({
            color: 'positive',
    ...
    const deleteTax = async () => {
      try {
        await api.delete(`/admin/${authStore.businessId}/taxes/${selectedTax.value.id}`)
        $q.notify({
          color: 'positive',
    ...
    const toggleTaxStatus = async (tax) => {
      try {
        await api.post(`/admin/${authStore.businessId}/taxes/${tax.id}/toggle`)
        $q.notify({
          color: 'positive',
