<template>
    <q-page padding>
      <div class="row q-mb-md items-center justify-between">
        <div class="text-h5">WareHouse Inventory</div>
        <q-btn
          color="primary"
          icon="add"
          label="Add Item"
  ...
  export default {
    ...
    setup() {
      ...
      const inventoryItems = computed(() => {
        console.log('this is the inventory data ', store.warehouseInventory)
        return store.warehouseInventory?.data || []
      })
      ...
      const loadData = async () => {
        ...
        try {
          const { businessId, branchId } = route.params
          await store.fetchWarehouseInventory(businessId, branchId, {
            filter: filter.value,
          ...
          pagination.value.rowsNumber = store.warehouseInventory?.data?.total || 0
        } catch {
          $q.notify({
            type: 'negative',
            message: 'Failed to load warehouse inventory items'
          })
        } finally {
          loading.value = false
        }
