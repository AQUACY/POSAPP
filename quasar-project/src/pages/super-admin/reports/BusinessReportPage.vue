<template>
  <q-page padding>
    <div class="row items-center justify-between q-mb-lg">
      <div class="text-h4">Business Reports</div>
      <div>
        <q-btn
          color="primary"
          icon="file_download"
    ...
    <div class="row q-col-gutter-md q-mb-lg">
      <div class="col-12 col-sm-6 col-md-3">
        <q-card class="glass-card">
          <q-card-section>
            ...
            <div class="text-h4 text-primary">${{ summary.totalRevenue }}</div>
            <div class="text-caption" :class="summary.revenueGrowth >= 0 ? 'text-positive' : 'text-negative'">
              {{ summary.revenueGrowth >= 0 ? '+' : '' }}{{ summary.revenueGrowth }}% from previous period
            </div>
          </q-card-section>
        </q-card>
      </div>
      ...
      <div class="col-12 col-sm-6 col-md-3">
        <q-card class="glass-card">
          <q-card-section>
            <div class="text-h6">Active Customers</div>
            <div class="text-h4 text-primary">{{ summary.activeCustomers }}</div>
            <div class="text-caption" :class="summary.customerGrowth >= 0 ? 'text-positive' : 'text-negative'">
              {{ summary.customerGrowth >= 0 ? '+' : '' }}{{ summary.customerGrowth }}% from previous period
            </div>
          </q-card-section>
        </q-card>
      </div>
...
const fetchReportData = async () => {
  loading.value = true
  try {
    const response = await superAdminStore.getBusinessReport(filters.value)
...
const exportReport = async () => {
  try {
    await superAdminStore.exportBusinessReport(filters.value)
  } catch (err) {
    $q.notify({
      color: 'negative',
